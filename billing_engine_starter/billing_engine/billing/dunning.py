...
class DunningProcess:
    ...
    def attempt(self, invoice: Invoice, customer_id: int, now: datetime) -> DunningOutcome:
        ...
        if result.success:
            ...
            self.ledger_repo.add(LedgerEntry(id=None,invoice_id=invoice.id,customer_id=customer_id,amount=invoice.total,direction=LedgerDirection.CREDIT,reason=f"payment for invoice{invoice.id}"))
